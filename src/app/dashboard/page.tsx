...
function PrimaryModuleCard({
  ...
  return (
    <a
      href={href}
      className="group rounded-2xl border border-emerald-100 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-md"
    >
      <div className="flex min-h-36 flex-col justify-between gap-4">
        ...
        <span className="inline-flex w-fit rounded-xl bg-emerald-700 px-4 py-2 text-sm font-bold text-white transition group-hover:bg-emerald-800">
          Abrir módulo
        </span>
      </div>
    </a>
...
export default async function DashboardPage() {
  ...
  const primaryOrder = ["/inventario", "/recetas-costos", "/produccion"];
  const primaryModules = primaryOrder
  ...
  const secondaryModules = visibleModules.filter(
    (module) => !primaryOrder.includes(module.href)
  );
